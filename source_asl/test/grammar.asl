...
C:\Acpi\source_asl\test>iasl -f -of grammar.asl

Intel ACPI Component Architecture
ASL Optimizing Compiler version 20060310 [Mar 10 2006]
Copyright (C) 2000 - 2006 Intel Corporation
Supports ACPI Specification Revision 3.0a

grammar.asl   392: NAME (ESC1, "abcdefg\x00hijklmn")
Warning  2041 -                            ^ Invalid Hex/Octal Escape - Non-ASCII or NULL

grammar.asl   393: NAME (ESC2, "abcdefg\000hijklmn")
Warning  2041 -                            ^ Invalid Hex/Octal Escape - Non-ASCII or NULL

grammar.asl   484: Method (RCIV, 1)
Warning  2086 -               ^ Not all control paths return a value (RCIV)

grammar.asl   491:     RCIV (Subtract (Arg0, 1))
Remark   3072 -           ^ Recursive method call (RCIV)

grammar.asl   604:             Method(_SRS)
Warning  2076 -                          ^ Reserved method has too few arguments (_SRS requires 1)

grammar.asl  1260:     Store (0x1234567887654321, QWD2)
Warning  2031 -                                ^ 64-bit integer in 32-bit table, truncating

grammar.asl  1262:     if (LNotEqual (Local0, 0x1234567887654321))
Warning  2031 -     64-bit integer in 32-bit table, truncating ^

grammar.asl  1689: Method (COND)
Warning  2086 -               ^ Not all control paths return a value (COND)

grammar.asl  5889:         Name (_HID, "*PNP0A06")
Error    1001 -                                 ^ String must be entirely alphanumeric (*PNP0A06)

grammar.asl  6341:         Name (_CRS, Buffer(26)  {"\_SB_.PCI2._CRS..........."})
Warning  2037 -    Invalid or unknown escape sequence ^

grammar.asl  6680:             And (Local0, 1, Local0) //  Local0 &= 1
Error    1049 -                          ^ Method local variable is not initialized (Local0)

grammar.asl  6766:         Name (_HID, "*PNP0C0A")     //  Control Method Battey ID
Error    1001 -                                 ^ String must be entirely alphanumeric (*PNP0C0A)

ASL Input:  grammar.asl - 10135 lines, 288243 bytes, 4797 keywords
AML Output: grammar.aml - 43282 bytes 663 named objects 4134 executable opcodes

Compilation complete. 3 Errors, 8 Warnings, 1 Remarks, 1100 Optimizations
...
//
//
// Imported from iPCO test IfElseOp.asl
//
...
//
// Imported from iPCO test NoSave.asl
//
//
...
//
// Imported from iPCO test IndxFld.asl
//
//  IndexFld test
...
//
// Imported from iPCO test NestdLor.asl
//
...
//
// Imported from iPCO test RetBuf.asl
//
...
//
// Imported from iPCO test RetLVal.asl
//
...
//
// Imported from iPCO test RetPkg.asl
//
...
//
// Imported from iPCO test WhileRet.asl
//
...
//
// Imported from iPCO test AndOrOp.asl
//
...
//) => Integer
//Source1 and Source2 are evaluated as integer data types, a bit-wise AND is performed, and the result is optionally
//stored into Result.
//
//
//Syntax of OrOp
...
//  Result  //Nothing | SuperName
//) => Integer
//Source1 and Source2 are evaluated as integer data types, a bit-wide OR is performed, and the result is optionally
//stored in Result
//
Device (ANDO)
{
...
//
// Imported from iPCO test BreakPnt.asl
//
// This code tests the BreakPoint opecode term. The syntax of BreakPoint Term is
//BreakPointTerm    := BreakPoint
//Used for debugging, the Breakpoint opcode stops the execution and enters the AML debugger.
//In the retail version of the interpreter, BreakPoint is equivalent to Noop.
//
Device (BRKP)
{
    Name(CNT0,0)
...
//
// Imported from iPCO test AddSubOp.asl
//
Device (ADSU)
{
    //  create System Memory Operation Region and field overlays
...
//
// Imported from iPCO test IncDecOp.asl
//
Device (INDC)
{
    //  create System Memory Operation Region and field overlays
...
//
// Imported from iPCO test LOps.asl
//
...
//
// Imported from iPCO test FdSetOps.asl
//
...
//
// Imported from iPCO test MulDivOp.asl
//
Device (MLDV)
{
    //  create System Memory Operation Region and field overlays
...
//
// Imported from iPCO test NBitOps.asl
//
//NAnd - Bit-wise NAnd
//NAndTerm  := NAnd(
//  Source1,    //TermArg=>Integer
...
//
// Imported from iPCO test ShftOp.asl
//
...
//
// Imported from iPCO test Xor.asl and slightly modified
//
...
//
// Imported from iPCO test CrBytFld.asl
//
//  CrBytFld test
...
//
// Imported from iPCO test IndexOp4.asl
//
//  IndexOp4 test
//      This is just a subset of the many RegionOp/Index Field test cases.
...
//
// Imported from iPCO test Event.asl
//
...
//
// Imported from iPCO test SizeOfLv.asl
//
...
//
// Imported from iPCO test BytField.asl
//
//  BytField test
//      This is just a subset of the many RegionOp/Index Field test cases.
//      Tests access of TBD.
...
//
// Imported from iPCO test DwrdFld.asl
//
...
//
// Imported from iPCO test DivAddx.asl
//
Name (B1LO, 0xAA)
Name (B1HI, 0xBB)
...
//
// Imported from iPCO test IndexFld.asl (IndexOp6.asl)
//
//  IndexFld test
//      This is just a subset of the many RegionOp/Index Field test cases.
//      Tests index field element AccessAs macro.
...
//
// Imported from iPCO test IndexOp5.asl
//
//  IndexOp5 test
//      This is just a subset of the many RegionOp/Index Field test cases.
...
//
// Imported from iPCO test IndexOp.asl
//
Scope (\_SB)    //  System Bus
{   //  _SB system bus
...
//
// Imported from iPCO test BitIndex.asl
//
//  BitIndex test
...
//
// Imported from iPCO test IndexOp3.asl
//
...
//
// Imported from iPCO test MatchOp.asl
//
...
//
// Imported from iPCO test WhileBrk.asl
//
...
//
// Imported from iPCO test IndexOp2.asl
//
//  Additional IndexOp test cases to support ACPICMB (control method battery
...
//
// Imported from iPCO test SizeOf.asl
//
//  Test for SizeOf
...
//
// Imported from iPCO test SmiShare.asl
//
Scope (\_SB)    //  System Bus
{   //  _SB system bus
